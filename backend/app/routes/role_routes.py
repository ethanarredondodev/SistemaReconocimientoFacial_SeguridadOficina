...
def update_role(role_id: int, update_data: RoleUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    ...
    if update_data.name is not None and update_data.name.strip() != "":
        ...
        if existing_name:
            raise HTTPException(
                status_code=400,
                detail="El nombre del role ya existe"
            )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Ocurrió un error mientras se actualizaba el role"
        )
...
def deactivate_role(role_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    ...
    if role.users:
        raise HTTPException(
            status_code=400,
            detail="No se puede desactivar el role porque está asignado a usuarios"
        )
    if role.role_offices:
        raise HTTPException(
            status_code=400,
            detail="No se puede desactivar el role porque está asignado a oficinas"
        )
    ...
    try:
        db.commit()
        return {"detail": "Role desactivado exitosamente"}
    except Exception:
        db.rollback()
        raise HTTPException(
            status_code=500,
            detail="Ocurrió un error mientras se desactivaba el role"
        )